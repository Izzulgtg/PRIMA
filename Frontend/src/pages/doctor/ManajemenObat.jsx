...
const ManajemenObat = () => {
  ...
  // Konfigurasi Token Auth (Sesuaikan jika kamu menyimpan token di localStorage dengan nama lain)
  ...
  const fetchObat = async () => {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        // Jika backend mengirim dalam objek: res.json({ data: hasilDatabase })
        setObatList(response.data.data);
      } else if (response.data && Array.isArray(response.data.obat)) {
        // Jika backend mengirim: res.json({ obat: hasilDatabase })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Gagal menyimpan data obat. Periksa koneksi atau kelengkapan data.');
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
              <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">
                    Batas Minimum Stok <span className="text-[10px] text-red-500 font-bold ml-1">PERINGATAN</span>
                  </label>
                ...
                <div>
                  ...
                  <input type="date" name="tanggal_kadaluarsa" value={formData.tanggal_kadaluarsa} onChange={handleInputChange} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#2A4736]" required />
                </div>
                <div>
              ...
              <div className="mt-6 bg-[#f4f7f5] rounded-xl p-4 flex gap-3 items-start border border-[#e6eee8]">
